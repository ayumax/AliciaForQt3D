        ...
        out vec3 vR1, out vec3 vR2, out float fDet)
{
	...
	mat3 view2obj = to_mat3(inverseModelViewMatrix);
...
void background_transform_to_world(vec3 viewvec, out vec3 worldvec)
{
	...
	vec4 co_homogenous = (inverseProjectionMatrix * v);

	vec4 co = vec4(co_homogenous.xyz / co_homogenous.w, 0.0);
	worldvec = (inverseModelViewMatrix * co).xyz;
}
        ...
        out vec3 camera, out vec3 window, out vec3 reflection)
{
	...
	co = normalize(co);
	vec3 coords = (inverseModelViewMatrix * co).xyz;
...
uniform vec3 unf16;
const float cons19 = float(1.000000000000);
uniform sampler2D samp1;
uniform float unf29;
const float cons30 = float(1.000000000000);
const float cons32 = float(1.000000000000);
uniform vec3 unf38;
uniform float unf49;
uniform float unf51;
uniform vec3 unf52;
const float cons57 = float(1.000000000000);
const vec4 cons61 = vec4(1.000000000000, 1.000000000000, 1.000000000000, 0.004999999888);
const float cons72 = float(1.000000000000);
uniform vec4 unf73;
const float cons76 = float(1.000000000000);
uniform vec3 unf79;
in vec3 varposition;
uniform float unf87;
uniform float unf88;
...
void main()
{
	...
	texco_uv(var0, tmp5);
	mtex_image(tmp5, samp0, tmp8, tmp9);
	set_rgba(tmp9, tmp11);
	mtex_alpha_from_col(tmp9, tmp13);
	srgb_to_linearrgb(tmp11, tmp15);
	mtex_rgb_mul(unf16, tmp15.rgb, tmp13, cons19, tmp20);
	texco_uv(var0, tmp22);
	mtex_image(tmp22, samp1, tmp25, tmp26);
	mtex_alpha_from_col(tmp26, tmp28);
	mtex_value_blend(unf29, cons30, tmp28, cons32, tmp33);
	mtex_value_clamp(tmp33, tmp35);
	set_rgb_zero(tmp36);
	set_rgb_zero(tmp37);
	lamp_visibility_sun_hemi(unf38, tmp39, tmp40, tmp41);
	shade_inp(tmp3, tmp39, tmp44);
	shade_is_hemi(tmp44, tmp46);
	shade_visifac(tmp46, tmp41, unf49, tmp50);
	shade_mul_value_v3(unf51, unf52, tmp53);
	shade_mul_value(tmp50, vec4(tmp53, 1.0), tmp56);
	mtex_value_invert(cons57, tmp58);
	mix_mult(tmp58, tmp56, cons61, tmp62);
	shade_madd(vec4(tmp36, 1.0), tmp62, vec4(tmp20, 1.0), tmp66);
	shade_clamp_positive(vec4(tmp37, 1.0), tmp68);
	shade_clamp_positive(tmp66, tmp70);
	shade_maddf(tmp70, cons72, unf73, tmp74);
	alpha_spec_correction(tmp68.rgb, cons76, tmp35, tmp78);
	shade_add_mirror(unf79, tmp1, tmp74.rgb, tmp82);
	mtex_alpha_to_col(vec4(tmp82, 1.0), tmp78, tmp85);
	shade_mist_factor(varposition, unf87, unf88, unf89, unf90, unf91, tmp92);
	mix_blend(tmp92, tmp85, unf95, tmp96);
	linearrgb_to_srgb(tmp96, tmp98);
