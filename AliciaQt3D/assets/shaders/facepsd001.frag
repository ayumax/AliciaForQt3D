...
uniform float unf35;
uniform vec3 unf36;
const float cons41 = float(1.000000000000);
const vec4 cons45 = vec4(1.000000000000, 1.000000000000, 1.000000000000, 0.004999999888);
const float cons54 = float(1.000000000000);
uniform vec4 unf55;
uniform vec3 unf57;
const float cons62 = float(1.000000000000);
in vec3 varposition;
uniform float unf65;
uniform float unf66;
uniform float unf67;
...
void main()
{
	...
	shade_inp(tmp3, tmp23, tmp28);
	shade_is_hemi(tmp28, tmp30);
	shade_visifac(tmp30, tmp25, unf33, tmp34);
	shade_mul_value_v3(unf35, unf36, tmp37);
	shade_mul_value(tmp34, vec4(tmp37, 1.0), tmp40);
	mtex_value_invert(cons41, tmp42);
	mix_mult(tmp42, tmp40, cons45, tmp46);
	shade_madd(vec4(tmp21, 1.0), tmp46, vec4(tmp20, 1.0), tmp50);
	shade_clamp_positive(tmp50, tmp52);
	shade_maddf(tmp52, cons54, unf55, tmp56);
	shade_add_mirror(unf57, tmp1, tmp56.rgb, tmp60);
	mtex_alpha_to_col(vec4(tmp60, 1.0), cons62, tmp63);
	shade_mist_factor(varposition, unf65, unf66, unf67, unf68, unf69, tmp70);
	mix_blend(tmp70, tmp63, unf73, tmp74);
	shade_alpha_opaque(tmp74, tmp76);
	linearrgb_to_srgb(tmp76, tmp78);
